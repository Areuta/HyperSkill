...
class MultifunctionalMapper {
    ...
    public static final Function<List<IntUnaryOperator>, UnaryOperator<List<Integer>>> MULTIFUNCTIONAL_MAPPER =
            // Stream API version of MULTIFUNCTIONAL_MAPPER
        ...
        /*    intUnaryOperatorsList -> integersList -> {
                List<Integer> retList = new ArrayList<>();
                for (Integer integer : integersList) {
                    Integer l0 = intUnaryOperatorsList.get(0).applyAsInt(integer);
                    for (int j = 1; j < intUnaryOperatorsList.size(); j++) {
                        l0 = intUnaryOperatorsList.get(j).applyAsInt(l0);
                    }
                    retList.add(l0);
                }
                return retList;
    ...
    public static final UnaryOperator<List<Integer>> MULT_TWO_AND_THEN_ADD_ONE_TRANSFORMATION =
            MULTIFUNCTIONAL_MAPPER.apply(Arrays.asList(x -> x, x -> 2 * x, x -> x + 1));
    /**
     * The operator accepts an integer list.
    ...
    public static final UnaryOperator<List<Integer>> SQUARE_AND_THEN_GET_NEXT_EVEN_NUMBER_TRANSFORMATION =
            MULTIFUNCTIONAL_MAPPER.apply(Arrays.asList(x -> x, x -> x * x, x -> x % 2 == 0 ? x + 2 : x + 1));
