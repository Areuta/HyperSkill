...
class Main {
    ...
    public static void printFilteredAccounts(List<Account> accounts) {
        Predicate<Account> check1 = account -> account.getBalance() > 0;
        Predicate<Account> check2 = account -> !account.isLocked() && account.getBalance() >= (long) 100_000_000;

        List<Account> nonEmptyAccounts = filter(accounts, account -> account.getBalance() > 0);
        List<Account> accountsWithTooMuchMoney =
                filter(accounts, account -> !account.isLocked() && account.getBalance() >= 100_000_000);
